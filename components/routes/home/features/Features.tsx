...
import { BookIcon, BoxIcon, LineIcon, MobileIcon, RoundIcon, TvIcon } from '../../../icon/myIcons'
...
const Features = () => {
    ...
    const data = [
        {
            icon: <LineIcon fontSize="large" color="primary" sx={{ mb: 1.5, mt: 3 }} />,
            title: 'Business Stratagy',
            body: 'I throw myself down among the tall grass by the stream as Ilie close to the earth.'
        },
        {
            icon: <BookIcon fontSize="large" color="primary" sx={{ mb: 1.5, mt: 3 }} />,
            ...
            <Grid container direction='row' spacing={4} sx={{ my: 3 }}>
                {data.map((value, index) => (
                    <Grid item key={index} xs={4}>
                        <Paper elevation={themeMode === "dark" ? 2 : 3} sx={{ borderRadius: '10px', cursor: 'pointer' }}>
                            <Stack sx={{ px: 6, py: 3 }} className='hover-up'>
                                {value.icon}
                                <Typography variant="h5" fontWeight={600} sx={{ my: 1 }}>{value.title}</Typography>
                                <Typography variant="body1"
                                    sx={{ lineHeight: '30px', color: 'text.secondary', my: 2, fontWeight: 500 }}>
                                    {value.body}
                                </Typography>
                                <ArrowForwardIcon
